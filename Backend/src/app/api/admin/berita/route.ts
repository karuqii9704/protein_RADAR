...
// GET /api/admin/berita - Get all news (KEGIATAN, PENGUMUMAN, LAPORAN)
export async function GET(request: NextRequest) {
  try {
    ...
    // Build where clause - only berita categories
    const where: {
...
// POST /api/admin/berita - Create news
export async function POST(request: NextRequest) {
  try {
    ...
    if (!BERITA_CATEGORIES.includes(category)) {
      return errorResponse('Invalid news category. Use KEGIATAN, PENGUMUMAN, or LAPORAN', 400);
    }
