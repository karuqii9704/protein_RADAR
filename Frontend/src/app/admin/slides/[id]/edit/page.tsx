...
export default function EditSlidePage() {
  ...
  useEffect(() => {
    const fetchSlide = async () => {
      try {
        const res = await apiGet<Slide>(`/api/admin/slides/${params.id}`);
        if (res.success && res.data) {
          ...
          if (slide.image) setImagePreview(slide.image);
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiPut(`/api/admin/slides/${params.id}`, {
        ...formData,
        linkUrl,
      });
      ...
      if (res.success) {
        ...
        toast.error(res.error || 'Gagal memperbarui slide');
      }
    } catch (error) {
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500">Perbarui slide carousel</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-6">
        ...
        <div className="flex items-center justify-end gap-4 pt-4 border-t border-gray-100">
          <Link href="/admin/slides" className="px-6 py-2.5 text-gray-600 hover:text-gray-800 font-medium transition">
            Batal
          </Link>
          <button
            type="submit"
